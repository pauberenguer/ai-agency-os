...
>
> La fuente de verdad sobre qué debería estar instalado es `~/.claude/skills/_install-state.json`. Si dice que `engine-core` está `done` pero la validación profunda falla, eso es **drift** y hay que reportarlo (y opcionalmente arreglarlo).
...
| **DRIFT check** | Si `_install-state.json.phases.engine-core.status == "done"` Y alguno de los anteriores falla → **DRIFT** | 🔴 con flag "STATE DRIFT" |
...
```
# 🩺 AI Agency OS · Health Check v0.6
...
- **Repo abierto pero no es ai-agency-os**: detecta ausencia de `vendor/sinapsis/` → "no estás en un repo ai-agency-os, este check no aplica"
