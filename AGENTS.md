...
> Este archivo lo lees TÚ (un agente IA) cuando alguien te pasa la URL de este repo o cuando abres el repo por primera vez. Define tu workflow para instalar y arrancar AI Agency OS de forma plug-and-play.
...
> "Instala AI Agency OS desde https://github.com/pauberenguer/ai-agency-os y guíame en el setup"

Variantes que también debes reconocer:
- "Instala el OS de AI Agency"
- "Configura mi sistema con AI Agency OS"
- "Quiero usar AI Agency OS"
- **"instala esto" · "instálamelo" · "monta el sistema" · "install this" · "set this up"** (dentro de la carpeta del repo)
- El comando `/instala`
- Cualquier URL del repo `pauberenguer/ai-agency-os`
...
```
Voy a instalar AI Agency OS en tu sistema. El proceso:

1. Clonar el repo en ~/ai-agency-os/ (~5 segundos)
...
```bash
cd ~ && git clone https://github.com/pauberenguer/ai-agency-os.git
cd ~/ai-agency-os
```

Si la carpeta ya existe, pregunta al usuario:
- "Ya tienes `~/ai-agency-os/`. ¿Lo actualizo (`git pull`), reinicio limpio (backup + clone fresh) o uso lo que hay?"
...
- **Si el usuario no es Angel**: SIEMPRE preguntar antes de tocar archivos fuera de `~/ai-agency-os/`. Sinapsis vive en `~/.claude/` — solo `install.sh` la toca
...
Abre un issue en https://github.com/pauberenguer/ai-agency-os/issues
