...
# AI Agency OS: el template vendored referencia rutas con la nomenclatura del
# engine. Se transforma a la propia ANTES del merge — así el dedup por comando
# compara contra las rutas renombradas y la idempotencia se mantiene.
TMP_TEMPLATE="$(mktemp "${TMPDIR:-/tmp}/settings-template.XXXXXX")"
perl -pe 's/sinapsis-learning/agency-learning/g; s/sinapsis-instincts/agency-instincts/g; s/_sinapsis-projects/_agency-projects/g; s/dashboard-sinapsis/dashboard-agency/g' "$TEMPLATE" > "$TMP_TEMPLATE"
