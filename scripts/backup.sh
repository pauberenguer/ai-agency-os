...
#    1. AI AGENCY_BACKUP_DIR del .env (elección del operador, se fija
#       la primera vez via /backup) o flag --dest <ruta>
#    2. Auto-detección: iCloud → Google Drive → Dropbox → OneDrive
#    3. $HOME/AI Agency-Backup/ (sin nube: solo copia local)
#
#  Uso:
...
# ── Resolver destino ──
# Prioridad: --dest > AI AGENCY_BACKUP_DIR (.env) > iCloud > Google Drive
...
if [ -f "$REPO_ROOT/.env" ]; then
    ENV_DEST=$(grep -E '^AI AGENCY_BACKUP_DIR=' "$REPO_ROOT/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' || true)
fi
...
if [ -n "$ARG_DEST" ]; then
    ...
    for cloud in icloud gdrive dropbox onedrive; do
        ...
        if [ -n "$base" ]; then
            DEST_ROOT="$base/AI Agency-Backup"
            break
        fi
    done
    [ -z "$DEST_ROOT" ] && DEST_ROOT="$HOME/AI Agency-Backup"
fi
...
if [ "${1:-}" = "--where" ]; then
    ...
    [ "$found" -eq 0 ] && echo "  (ninguna — el backup iría a $HOME/AI Agency-Backup)"
    echo ""
    echo -e "Destino actual: ${CYAN}$DEST_ROOT${NC}"
    echo "Para fijar otro: añade AI AGENCY_BACKUP_DIR=<ruta> al .env (o dile a Claude dónde lo quieres)."
    exit 0
fi
...
echo -e "${PURPLE}${BOLD}  AI Agency OS — Backup${NC}"
echo -e "${PURPLE}${BOLD}============================================================${NC}"
echo ""
