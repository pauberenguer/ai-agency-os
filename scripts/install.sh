#!/bin/bash
# ============================================================
#  AI Agency OS — Installer v0.6 (state-machine, reentrant)
#  Sistema operativo agéntico para operadores de IA
#  https://github.com/pauberenguer/ai-agency-os
# ============================================================
#
# Cambios vs v0.5:
...
echo ""
echo "============================================================"
echo "  AI Agency OS — Installer v0.6.0"
echo "  Repo: $REPO_ROOT"
echo "============================================================"
echo ""
...
record_sinapsis_validation() {
    ...
    local skill_count
    skill_count=$(find -L "$SKILLS_DIR" -maxdepth 3 -name "SKILL.md" 2>/dev/null | wc -l | tr -d ' ')
    json_set_phase "engine-core" "validation" \
...
phase_sinapsis_engine() {
    local current_status
    current_status=$(json_get_phase_status "engine-core")
    if [ "$current_status" = "done" ] && ! $FORCE_REINSTALL; then
        # Aún así re-validamos para detectar drift (alguien borró archivos manualmente)
        if validate_sinapsis_deep >/dev/null 2>&1; then
            record_sinapsis_validation
            clear_phase_errors "engine-core"  # purga fantasmas de intentos viejos ya resueltos
            skip "engine-core · ya instalado y validado (status=done)"
            return 0
        else
            warn "engine-core · marcado done pero validación profunda falla · re-instalando"
            json_set_phase "engine-core" "status" '"in-progress"'
        fi
    fi
    ...
    if $SKIP_SINAPSIS; then
        warn "engine-core · saltado por --skip-sinapsis (solo debug)"
        return 0
    fi

    if [ ! -d "$SINAPSIS_VENDOR" ]; then
        mark_phase_failed "engine-core" "vendor/sinapsis/ no encontrado · repo incompleto, vuelve a clonar"
    fi
    if [ ! -f "$SINAPSIS_VENDOR/install.sh" ]; then
        mark_phase_failed "engine-core" "vendor/sinapsis/install.sh no encontrado · vuelve a clonar el repo"
    fi
    ...
    if ! bash install.sh; then
        cd "$prev_dir"
        mark_phase_failed "engine-core" "vendor/sinapsis/install.sh devolvió error · revisa el output anterior"
    fi
    cd "$prev_dir"

    # AI Agency OS: renombrar los artefactos del engine a nomenclatura propia.
    # Corre ANTES de cablear hooks: el cableado ya escribe rutas renombradas.
    info "Aplicando nomenclatura AI Agency OS al engine..."
    bash "$SCRIPT_DIR/_debrand-engine.sh" || warn "debrand incompleto · ejecuta scripts/_debrand-engine.sh a mano"
    ...
    info "Cableando hooks del engine en ~/.claude/settings.json..."
    if ensure_sinapsis_hooks; then
        ok "Hooks del engine cableados en settings.json"
    else
    ...
    info "Validando instalación del engine (validación profunda)..."
    if ! validate_sinapsis_deep; then
        record_sinapsis_validation
        mark_phase_failed "engine-core" "Sinapsis se ejecutó pero la validación profunda falla · ver warnings arriba"
    fi

    record_sinapsis_validation
    compute_and_store_checksum
    mark_phase_done "engine-core"
    ok "Engine instalado y validado"
}
...
EOF
    ...
    json_set_phase "engine-core" "validation" "$validation_json"
    json_set_phase "engine-core" "checksum" "\"sha256:$files_hash\""
}
...
try {
  ...
  s.hooks.SessionStart.push({
    hooks: [
      {
        _comment: 'AI Agency OS install gate: bloquea sesiones si _install-state.json no completo',
        type: 'command',
...
main() {
    ...
    echo ""
    echo "============================================================"
    echo "  ✓ Fases técnicas completadas (prereqs + engine-core + capa OS)"
    echo "============================================================"
    echo ""
    echo "  Estado actual:"
    case "$JSON_RUNTIME" in
